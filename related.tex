...
in many disciplines Lazer~\textit{et al}~\cite{lazer09}. Much of this work has studied
questions in sociology and human interaction, such as how friendships
form Crandall~\textit{et al}~\cite{feedback08kdd}, how information flows through social
networks Liben-Nowell and Kleinberg~\cite{libennowell08}, how people move through
space Brockmann~\textit{et al}~\cite{brockmann06}, and how people influence their
peers Anagnostopoulos~\textit{et al}~\cite{anagnostpopoulos08}.  The goal of these projects is not to
...
source of observational data about the world itself.  Bollen~\textit{et al}~\cite{bollen11twitter} use data from Twitter to try to measure
...
the world itself, as a special case of `social sensing' Aggarwal~\textit{et al}~\cite{Aggarwal:2013vh}.
This work includes using Twitter data to measure collective emotional
state Golder~\textit{et al}~\cite{Golder:2011cy} (which, in turn, has found to
be predictive of stock moves Bollen~\textit{et al}~\cite{bollen11twitter}),
predicting product adoption rates and political election
outcomes Jin~\textit{et al}~\cite{jin10prediction}, and collecting data about
earthquakes and other natural disasters Sakaki~\textit{et al}~\cite{Sakaki:2010uv}.
...
Over a sample of 10 million images on Flickr.com, 37\% (only a small subset) of them probably have incorrect timestamp Thomee~\textit{et al}~\cite{thomee2014s}.
...
Singh~\textit{et al}~\cite{singh2010geo}estimates where images are taken for those missing geo-tags. They are optimizing a graph clustering problem. Attributes in their graph include textual tags, timestamps and vision content. It's inspired by an earlier work Crandall~\textit{et al}~\cite{crandall2009mapping}.
...
Thomee~\textit{et al}~\cite{thomee2014s}  show  a detailed analysis in disagreement of camera time and GPS time. They also estimate a more accurate timestamp when users taking multiple images in a short timespan.
Hauff~\textit{et al}~\cite{hauff2012geo} consider textual meta-data to correct geo tags. 
...
in observations~\cite{greatsunflower}, Fink~\textit{et al}~\cite{ebirds}, King~\textit{et al}~\cite{king09snowtweets}. These projects
...
\textit{et al}~\cite{singhal2003spatialcontext}, Luo~\textit{et al}~\cite{luo2003spatialcontext}
...
most difficult.  Follow-up work Boutell~\textit{et al}~\cite{boutell2006semanticfeature}, Boutell~\cite{boutell2005exploiting}
...
Kumar~\textit{et al}~\cite{kumar2012leafsnap}identifies plant species by leaf images. They focus on accurate leaf segmentation according to color difference of leaf and background, curvature distribution over scale, and nearest neighbor matching.

%~\cite{Comparisons of Gist Models in Rapid Scene categorization tasks}

Siagian~\textit{et al}~\cite{siagian2008comparison}introduces multiple Gist models in scene classification. There happen to be a test set of vegetation shows Gist feature works great on vegetation classification.

%~\cite{Detecting subpixel woody vegetation in digital imagery using two artificial intelligence approaches}

Foschi~\textit{et al}~\cite{foschi1997detecting} is using man-made mask and neural networks.
...
This paper Balamurugan~\textit{et al}~\cite{balamurugan2007greenery}
